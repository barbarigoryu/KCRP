...
| 에이전트 | 모델 | 역할 |
|---|---|---|
| **Master** | Haiku 4.5 | 단계별 Key Action 달성 여부 판정 → 단계 자동 진행 |
| **Persona** | Haiku 4.5 | AI 고객 역할 (1~2문장 짧은 응답) |
| **Hint** | Haiku 4.5 | 실시간 코칭 힌트 (대화 흐름과 별도) |
| **Evaluation** | Sonnet 4.6 | 세션 종료 후 단계별 체크리스트 평가 |
| **Recommendation** | Haiku 4.5 | 약점 분석 기반 맞춤 훈련 추천 |
...
각 스크립트 상단의 버킷명을 실제 값으로 교체합니다.

**`setup_apigw.py`**
```python
region = 'ap-northeast-2'
fn     = 'kt-sales-training-chatbot'
bucket = 'kt-sales-training-web-<your-suffix>'   # ← 교체
```
...
WEB_BUCKET  = 'kt-sales-training-web-<your-suffix>'   # ← 교체
API_NAME    = 'kt-sales-training-api'
```

**`deploy_frontend.ps1`** (배포 스크립트도 동일하게 교체)
```powershell
param(
    [string]$Bucket = "kt-sales-training-web-<your-suffix>"   # ← 교체
    ...
)
```
...
이 스크립트가 수행하는 작업:
- 프라이빗 데이터 버킷 자동 생성 (`kt-sales-training-data-<account_id_suffix>`)
- Lambda 환경변수 설정 (`DATA_BUCKET`, `TRAINING_API_KEY`)
- 프론트엔드 HTML에 API URL·Key 주입 후 S3 업로드

출력 예시:
```
[INFO] Data bucket target: kt-sales-training-data-<suffix>
...
`manylinux2014_x86_64` 호환 패키지를 빌드하여 Lambda에 업로드합니다.
...
Lambda에서 API URL·API Key를 자동으로 읽어 HTML에 주입한 뒤 S3에 업로드합니다.
...
| 훈련 기록 | `training-results.html` | 이력 조회·통계·대화 보기 모달 |
...
- `setup_apigw.py`, `setup_analyzer.py`, `deploy_frontend.ps1` 상단의 버킷명(`<your-suffix>`)을 실제 버킷명으로 교체해야 합니다.
- AWS 계정 ID는 STS(`get_caller_identity`)로 자동 조회합니다 — 하드코딩 불필요.
...
- `sessionStorage`를 활용해 브라우저 새로고침 시 진행 중인 훈련을 복구합니다.
